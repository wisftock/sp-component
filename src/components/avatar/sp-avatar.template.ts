...
export function avatarTemplate(this: SpAvatarComponent): TemplateResult {
  return html`
    <div class="sp-avatar-wrapper">
      <div class="sp-avatar" style="background: var(--sp-avatar-bg); color: var(--sp-avatar-color);">
        ${this.loading
          ? html`<span class="sp-avatar-shimmer" aria-hidden="true"></span>`
      ...
      ${this.status
        ? html`<span class="sp-avatar-status sp-avatar-status--${this.status}" aria-label=${this.status}></span>`
        : nothing}
    </div>
  `;
}
