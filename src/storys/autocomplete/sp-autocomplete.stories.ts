...
export const GroupedCountries: Story = {
  name: "Single — Countries grouped by region",
  render: () => html`
    <sp-autocomplete
...
export const Creatable: Story = {
  ...
  render: () => {
    const onSearch = (e: Event) => console.log("sp-search", (e as CustomEvent).detail);
    ...
    return html`
      <div style="display: flex; flex-direction: column; gap: 12px; max-width: 380px;">
        <sp-autocomplete
          .options=${FRUIT_OPTIONS}
          label="Fruit"
          hint='Type a fruit name not in the list and select "Create"'
        ...
        <div id="create-output" style="
          ...
          background: #F6FFED; color: #389E0D; border: 1px solid #D9F7BE;
          min-height: 36px;
...
export const AsyncSearch: Story = {
  ...
  render: () => {
    ...
    const ALL_OPTIONS = [...FRUIT_OPTIONS];

    const onSearch = (e: Event) => {
      const el = e.currentTarget as HTMLElement & { loading: boolean; options: typeof ALL_OPTIONS };
      ...
      timer = setTimeout(() => {
        el.loading = false;
        el.options = ALL_OPTIONS.filter(o => o.label.toLowerCase().includes(query.toLowerCase()));
      }, 600);
    };
    ...
    return html`
      <sp-autocomplete
        .options=${ALL_OPTIONS}
        label="Fruit (async)"
        hint="Results load after 600ms debounce"
        placeholder="Type to search..."
        filter-mode="none"
        debounce="300"
        clearable
...
export const MultipleDefault: Story = {
  ...
  render: () => html`
    <sp-autocomplete
      ...
      hint="Search and select multiple fruits. Backspace removes the last tag."
  ...
  render: () => html`
    <div style="display: flex; flex-direction: column; gap: 20px; max-width: 440px;">
      <sp-autocomplete
        .options=${FRUIT_OPTIONS}
        .values=${["apple", "banana"]}
        size="sm" label="Small" multiple
      ></sp-autocomplete>
      <sp-autocomplete
        .options=${FRUIT_OPTIONS}
        .values=${["apple", "banana"]}
        size="md" label="Medium" multiple
      ></sp-autocomplete>
      <sp-autocomplete
        .options=${FRUIT_OPTIONS}
        .values=${["apple", "banana"]}
        size="lg" label="Large" multiple
      ></sp-autocomplete>
    </div>
  `,
};
